...
Example (train + validate):
    python train_heads.py \
        ...
        --validate \
        --val-samples 256 \
        --tsmixup-cache-dir "data/tsmixup_cache/" \
        --tsmixup-processed-cache "data/tsmixup_cache/tsmixup_processed_300000_512_128.pkl"
...
class TrainConfig:
    ...
    # "moirai", "studentst", "gaussian"
    mixture_components: List[str] = None # e.g., ["moirai", "students", "gaussian"]
    mixture_K: List[str] = None # e.g., [5, 5, 5]
...
class HeadTrainer:
    ...
    def _build_head(self, head_type: str, mixture_component: str, mixture_K: int) -> Tuple[nn.Module, Optional[MixtureOutput]]:
        """
        Build the prediction head and (optionally) a MixtureOutput spec for mixture loss.
        """
        if head_type == "mixture":
            if mixture_component == "moirai":
            ...
            elif mixture_component == "gaussian":
                components = [NormalOutput() for _ in range(mixture_K)]
            elif mixture_component == "studentst":
                components = [StudentTOutput() for _ in range(mixture_K)]
    ...
    def _head_save_path(self, head_type: str, mixture_component: str, mixture_K: int) -> str:
        if (head_type == "mixture") and (mixture_component != "moirai"):
            return os.path.join(self.cfg.save_dir, f"{self.cfg.save_prefix}_{head_type}_{mixture_component}_{mixture_K}_head.pt")
        return os.path.join(self.cfg.save_dir, f"{self.cfg.save_prefix}_{head_type}_head.pt")

    # ---------------------------- Training ---------------------------- #

    def train_head(self, head_type: str, mixture_component: str, mixture_K: int) -> Dict[str, float]:
        """
        Train a single head type and save checkpoint. Returns a dict of summary stats (e.g., median loss hist tail).
        """
        output_dims = self.OUTPUT_DIMS[head_type]
        pred_head, mixture_output = self._build_head(head_type, mixture_component, mixture_K)
        ...
        # Save head
        ckpt_path = self._head_save_path(head_type, mixture_component, mixture_K)
    ...
    def run(self) -> List[Dict[str, float]]:
        ...
        for head_type, mixture_component, K in zip(self.cfg.head_types, self.cfg.mixture_components, self.cfg.mixture_K):
            print(f"\n=== Training head: {head_type} {mixture_component} {K}===")
            summary = self.train_head(head_type, mixture_component, K)
            ...
            if self.cfg.validate:
                print(f"--- Validating head: {head_type} {mixture_component} {K} on tsmixup ---")
                try:
...
def parse_args() -> TrainConfig:
    ...
    parser.add_argument("--heads", type=str, nargs="+", default=[],
                        help="Head types to train sequentially. Choices: mse gaussian poisson neg_binom studentst quantiles mixture")
    parser.add_argument("--mixture_components", type=str, nargs="+", default=[],
                        help="Component distributions if using mixture head. Array must be same length as args.heads.  Choices: moirai, gaussian, studentst")
    parser.add_argument("--mixture_K", type=int, nargs="+", default=[],
                        help="Number of Component distributions if using mixture head. Array must be same length as args.heads. Integers")
    ...
    if "mixture" not in args.heads:
        args.mixture_components = [""] * len(args.heads)
        args.mixture_K = [""] * len(args.heads)
    elif (len(args.mixture_K) != len(args.heads)) or \
        (len(args.mixture_components) != len(args.heads)):
        raise Exception("args.heads must be the same length as mixture_K and mixture_components")
    cfg = TrainConfig(
