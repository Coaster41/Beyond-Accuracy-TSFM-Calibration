...
def latent_forecast(pred_head: nn.Module, latents: Tensor, horizon_len: int, head_type: str, 
    ...
    if output_dims == None:
        ...
        output_dims = output_dims_dict.get(head_type, output_dims_dict["mixture"])
    
    # if head_type != "mixture":
    if "mixture" not in head_type:
...
class HeadForecaster:
    ...
    def load_heads(self, head_types: List[str], mixture_components: List[str], 
                   mixture_K: List[int], ckpt_paths: Optional[Dict[str, str]] = None):
        """
        ...
        for h, mixture_component, K in zip(head_types, mixture_components, mixture_K):
            head, mixture_output = self._build_head(h, mixture_component, K)
            # pick checkpoint path
            if ckpt_paths and h in ckpt_paths:
                if h == "mixture":
                    warnings.warn(f"mixture in ckpt_paths may be using the incorrect path: {ckpt_paths[h]}")
                ckpt = ckpt_paths[h]
            else:
                if (h == "mixture") and (mixture_component != "moirai"):
                    ckpt = os.path.join(self.cfg.ckpt_dir, f"{self.cfg.ckpt_prefix}_{h}_{mixture_component}_{K}_head.pt")
                else:
    ...
    def forecast(self, context: Tensor, heads: Optional[List[str]] = None) -> Dict[str, Dict[str, Tensor]]:
        ...
        for h in heads:
            ...
            output_dims = self.OUTPUT_DIMS.get(h, self.OUTPUT_DIMS["mixture"])
            with torch.no_grad():
