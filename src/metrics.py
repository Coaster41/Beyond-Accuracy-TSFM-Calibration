...
def metric_pce(q_aligned: pd.DataFrame) -> Tuple[pd.Series, float]:
    ...
    grp = q_aligned.assign(ind=indicators).groupby(["q"])["ind"].mean()
    grp_h = q_aligned.assign(ind=indicators).groupby(["h", "q"])["ind"].mean()
    # Absolute difference from nominal q
    pce_hq = np.abs(grp_h.index.get_level_values("q") - grp_h.values)
    pce_q = np.abs(grp.index.get_level_values("q") - grp.values)
    # Average across q per h
    # Build a Series indexed by (h,q), then group by h
    pce_h = pd.Series(pce_hq, index=grp_h.index).groupby(level="h").mean().sort_index()
    pce = pd.Series(pce_q, index=grp.index).mean()
    return pce_h, pce
...
def metric_pce_by_series(q_aligned: pd.DataFrame) -> pd.Series:
    ...
    """
    indicators = (q_aligned["y"] <= q_aligned["yhat"]).astype(float)
    grp_h = q_aligned.assign(ind=indicators).groupby(["unique_id", "h", "q"])["ind"].mean()
    pce_uhq = pd.Series(np.abs(grp_h.index.get_level_values("q") - grp_h.values), index=grp_h.index)
    pce_uh = pce_uhq.groupby(level=["unique_id", "h"]).mean().sort_index()

    grp = q_aligned.assign(ind=indicators).groupby(["unique_id", "q"])["ind"].mean()
    pce_uq = pd.Series(np.abs(grp.index.get_level_values("q") - grp.values), index=grp.index)
    pce_u = pce_uq.groupby(level=["unique_id"]).mean().sort_index()
    return pce_uh, pce_u
...
def metric_tpce(panel_lu: pd.DataFrame, confidence: float) -> Tuple[pd.Series, float]:
    ...
    upper_out = (panel_lu["y"] > panel_lu["upper"]).mean()
    lower_out = (panel_lu["y"] < panel_lu["lower"]).mean()
    tpce = 0.5 * (np.abs(outside_ratio - upper_out) + np.abs(outside_ratio - lower_out))
    return tpce_h, tpce
...
def metric_tpce_by_series(panel_lu: pd.DataFrame, confidence: float) -> pd.Series:
    ...
    by_u = panel_lu.groupby(["unique_id"])
    upper_out = by_u.apply(lambda g: (g["y"] > g["upper"]).mean())
    lower_out = by_u.apply(lambda g: (g["y"] < g["lower"]).mean())
    tpce_u = 0.5 * (np.abs(outside_ratio - upper_out) + np.abs(outside_ratio - lower_out))
    return tpce_uh.sort_index(), tpce_u.sort_index()
...
def compute_all_metrics(
    ...
    pce_uh, pce_u = metric_pce_by_series(q_aligned)
    tpce_uh, tpce_u = metric_tpce_by_series(panel_lu, confidence=confidence)
    ...
    per_series_summary["PCE"] = pce_u
    per_series_summary["TPCE"] = tpce_u
