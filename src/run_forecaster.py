...
def load_test_data(
    ...
    if unit == "M":
        ...
        total_forecast_length = (end_date - forecast_date_ts) // freq_delta + 1
...
def parse_args():
    ...
    parser.add_argument("--heads", type=str, nargs="*",
                        default=[],
                        help="Heads to evaluate; 'backbone' is added automatically. e.g. ['quantiles', 'studentst', 'gaussian', 'mixture']")
    ...
    # Redundant...
    parser.add_argument("--include-backbone", action="store_true", default=False,
...
def main():
    ...
    mixture_components: List[str] = list(args.mixture_components)
    mixture_K: List[str] = list(args.mixture_K)
    # Important: do not try to load "backbone" as a head
    heads_to_load = [h for h in heads if h != "backbone"]
    loaded_heads = forecaster.load_heads(heads_to_load, mixture_components, mixture_K)
    print("Loaded heads:", loaded_heads)

    if args.include_backbone and "backbone" not in loaded_heads:
        loaded_heads.append("backbone")
    ...
    forecasts = {h: [] for h in loaded_heads}
    ...
    for i, batch in enumerate(batcher(test_data.input, batch_size=args.batch_size)):
        ...
        if args.ar:
            ...
            req_heads = [h for h in loaded_heads if h != "backbone"]
